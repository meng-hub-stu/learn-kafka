...
//        consumer.subscribe(compile("topic-*"));
        //使用partition的进行获取数据
//        consumer.assign(Collections.singletonList(new TopicPartition(TOPIC_CORE, 0)));
        //获取想要使用的partition数据
        List<PartitionInfo> partitionInfos = consumer.partitionsFor(TOPIC_CORE);
        List<TopicPartition> partitions = Lists.newArrayList();
        for (PartitionInfo pi : partitionInfos) {
            int partition = pi.partition();
            System.out.println(String.format("获取的分区：%s,获取的patition：%s", TOPIC_CORE, partition));
            partitions.add(new TopicPartition(TOPIC_CORE, partition));
        }
